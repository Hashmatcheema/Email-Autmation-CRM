...
export default function SalesActivityPage() {
  ...
  useEffect(() => {
    ...
    async function loadTodayStats() {
      ...
      const acts = (data as { activity_type: string }[]) ?? []
      setTodayStats({
        total: acts.length,
  ...
  useEffect(() => {
    ...
    void load()
  }, [profile, page, datePreset, activityTypeFilter, performedByFilter, refreshKey])
  ...
  const STAT_CARDS = [
    { label: 'Activities Today', value: todayStats.total, color: 'text-slate-700' },
    { label: 'Emails Sent Today', value: todayStats.emails, color: 'text-blue-700' },
    { label: 'Leads Created Today', value: todayStats.leadsCreated, color: 'text-emerald-700' },
    { label: 'Stage Changes Today', value: todayStats.stageChanges, color: 'text-amber-700' },
    { label: 'Notes Added Today', value: todayStats.notes, color: 'text-violet-700' },
  ]
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="mt-0.5 text-xs text-slate-500">All salesperson activity across the CRM</p>
      </div>
